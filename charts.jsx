// Hand-rolled SVG charts for rent vs buy.
const { useMemo } = React;
...
function CumulativeChart({ data, accent }) {
  const W = 720, H = 320;
  ...
  const linePath = (key) => {
    return data.map((d, i) => `${i === 0 ? 'M' : 'L'} ${x(d.year)} ${y(d[key])}`).join(' ');
  };
  const areaPath = (key) => {
    return `${linePath(key)} L ${x(maxX)} ${PAD_T + innerH} L ${PAD_L} ${PAD_T + innerH} Z`;
  };
    ...
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
      ...
      {yTicks.map((t, i) => (
        <g key={i}>
          ...
          <text x={PAD_L - 8} y={y(t) + 4} textAnchor="end" fontSize="11" fill="#0F0F0E" opacity="0.55" fontFamily="Inter, sans-serif">{fmt$(t)}</text>
        </g>
      ))}
      {xTicks.map((t, i) => (
        <text key={i} x={x(t)} y={H - 14} textAnchor="middle" fontSize="11" fill="#0F0F0E" opacity="0.55" fontFamily="Inter, sans-serif">Yr {t}</text>
      ))}
      {/* Areas */}
      ...
      <g>
        ...
        <text x={x(maxX) - 8} y={y(data[data.length-1].buyCumCost) - 8} textAnchor="end" fontSize="12" fontWeight="600" fill={accent} fontFamily="Inter, sans-serif">Buy {fmt$(data[data.length-1].buyCumCost)}</text>
        <circle cx={x(maxX)} cy={y(data[data.length-1].rentCumCost)} r="4" fill="#0F0F0E" />
        <text x={x(maxX) - 8} y={y(data[data.length-1].rentCumCost) - 8} textAnchor="end" fontSize="12" fontWeight="600" fill="#0F0F0E" fontFamily="Inter, sans-serif">Rent {fmt$(data[data.length-1].rentCumCost)}</text>
      </g>
    </svg>
  );
}
...
function NetWorthChart({ data, accent, breakEvenYear }) {
  const W = 720, H = 320;
  const PAD_L = 56, PAD_R = 24, PAD_T = 24, PAD_B = 36;
  const innerW = W - PAD_L - PAD_R, innerH = H - PAD_T - PAD_B;
    ...
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
      ...
      {breakEvenYear && (
        <g>
          ...
          <text x={x(breakEvenYear)} y={PAD_T + 10} textAnchor="middle" fontSize="11" fontWeight="600" fill="#fff" fontFamily="Inter, sans-serif">Break-even</text>
        </g>
      )}
      ...
      <g>
        ...
        <text x={x(maxX) - 8} y={y(data[data.length-1].buyWealth) - 8} textAnchor="end" fontSize="12" fontWeight="600" fill={accent} fontFamily="Inter, sans-serif">Buy {fmt$(data[data.length-1].buyWealth)}</text>
        <circle cx={x(maxX)} cy={y(data[data.length-1].rentWealth)} r="4" fill="#0F0F0E" />
        <text x={x(maxX) - 8} y={y(data[data.length-1].rentWealth) - 8} textAnchor="end" fontSize="12" fontWeight="600" fill="#0F0F0E" fontFamily="Inter, sans-serif">Rent {fmt$(data[data.length-1].rentWealth)}</text>
      </g>
    </svg>
  );
}

// MONTHLY COST BREAKDOWN (stacked bar, buy vs rent)
function BreakdownChart({ breakdown, accent }) {
  const buyParts = [
  ...
  const Bar = ({ parts, total, label, single }) => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
        ...
        <div style={{ fontFamily: 'Instrument Serif, serif', fontSize: 28, fontWeight: 400 }}>{fmt$0(total)}<span style={{ fontSize: 13, opacity: 0.5, marginLeft: 4 }}>/mo</span></div>
      </div>
      <div style={{ display: 'flex', height: 44, width: `${(total/max)*100}%`, minWidth: 60, borderRadius: 4, overflow: 'hidden' }}>
        {single ? (
          <div style={{ flex: 1, background: '#0F0F0E', display: 'flex', alignItems: 'center', paddingLeft: 12, color: '#fff', fontSize: 12, fontWeight: 500 }}>Rent payment</div>
        ) : parts.map((p, i) => (
          <div key={i} style={{ width: `${(p.value/total)*100}%`, background: p.color, position: 'relative' }} title={`${p.label}: ${fmt$0(p.value)}`}>
            {(p.value/total) > 0.12 && (
              <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', paddingLeft: 8, color: '#fff', fontSize: 11, fontWeight: 500, mixBlendMode: i === 0 ? 'normal' : 'normal' }}>
                {p.label}
              </div>
            )}
          </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 16px', fontSize: 12, opacity: 0.7, marginTop: 4 }}>
          {parts.map((p, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ width: 8, height: 8, background: p.color, borderRadius: 2, display: 'inline-block' }}></span>
              <span>{p.label} {fmt$0(p.value)}{p.note && <span style={{ opacity: 0.6 }}> · {p.note}</span>}</span>
            </div>
          ))}
        </div>
      )}
...
window.NetWorthChart = NetWorthChart;
